                    ...
                    <form action="{{ route('cabinet.faculties.update', $faculty->id) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
                            <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', $faculty->name) }}" class="mt-2 block w-full rounded-md border-gray-300 shadow-sm">
                            ...
                            <div>
                                ...
                                <select name="university_id" id="university_id" class="mt-2 block w-full rounded-md border-gray-300 shadow-sm">
                                    ...
                                    @foreach($universities as $university)
                                        <option value="{{ $university->id }}" @selected(old('university_id', $faculty->university_id) == $university->id)>{{ $university->name }}</option>
                                    @endforeach
                                </select>
