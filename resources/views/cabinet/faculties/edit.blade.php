        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="text-2xl font-semibold mb-6">Редактирование факультета</h3>

                    <form action="{{ route('cabinet.faculties.update', $faculty->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
                            <div>
                                <label for="name_faculty" class="block text-sm font-semibold text-gray-900">Название</label>
                                <input type="text" name="name_faculty" id="name_faculty" value="{{ old('name_faculty', $faculty->name_faculty) }}" class="mt-2 block w-full rounded-md border-gray-300 shadow-sm">
                            ...
                            <div>
                                ...
                                <select name="id_university" id="id_university" class="mt-2 block w-full rounded-md border-gray-300 shadow-sm">
                                    ...
                                    @foreach($universities as $university)
                                        <option value="{{ $university->id }}" @selected(old('id_university', $faculty->id_university) == $university->id)>{{ $university->name_university }}</option>
                                    @endforeach
                                </select>
