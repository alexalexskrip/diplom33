                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            @foreach($project->media as $media)
                                <div class="border rounded overflow-hidden cursor-pointer" x-data="{ open: false }" @keydown.window.escape="open = false">
                                    <img @click="open = true" src="{{ asset('storage/projectmedia/' . $media->file_path) }}" alt="" class="object-cover w-full h-48">
                                    <div class="text-sm text-center py-1 bg-gray-100">#{{ $media->position }}</div>
                                    ...
                                    <template x-if="open">
                                        ...
                                        >
                                            <div class="relative">
                                                ...
                                                <img src="{{ asset('storage/projectmedia/' . $media->file_path) }}" alt="" class="max-w-3xl max-h-[90vh] shadow-lg rounded">
                                            </div>
                                        </div>
                                    </template>
                                </div>
